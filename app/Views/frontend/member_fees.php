    ...
    <div class="container-fluid p-0 nav-bar">
        <nav class="navbar navbar-expand-lg bg-none navbar-dark py-3">
            <a href="<?= base_url('home/index'); ?>" class="navbar-brand">
                <h1 class="display-4 font-weight-bold text-uppercase text-white shadow bg-body rounded logo">Fitness
                    Club</h1>
            </a>
        </nav>
    </div>
                    ...
                    <div class="row">
                        <div class="col-lg-10 col-md-10 log-det">
                            <div class="small-logo">
                                <i class="fa-solid fa-dumbbell"></i> <span class="fw-bold">Fitness Club
                                    Membership</span>
                            </div>
                            <p class="dfmn">Select membership duration and Proceed to pay</p>
                            <div>
                                <?php if(!empty(session()->getTempdata('wrong'))){ ?>
                                <div class="alert alert-dark text-center text-white font-weight-bold">
                                    <?= session()->getTempdata('wrong');  ?></div>
                                <?php } ?>
                            </div>
                            <form method="post" action="<?= base_url('payment/pay_process'); ?>" id="form_payment">
                                <input type="hidden" id='user_id' name='user_id'
                                    value="<?= session()->getTempdata('user_id');  ?>" />
                                <div class="text-box-cont">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <?php  ?>
                                        <input type="text" class="form-control" readonly placeholder="Fees" value=""
                                            id="member_fees">
                                    </div>

                                    <div class="input-group center">
                                        <a href="<?= base_url('home/index'); ?>"
                                            class=" btn btn-dark btn-round text-white proceed" data-bs-toggle="modal"
                                            data-bs-target="#staticBackdrop" name="pay">Proceed

                                            to Pay</a>
                                    </div>
                                    <div class="row">
                                        <p class="forget-p">do you wan pay later? <a
                                                href="<?= base_url('home/index'); ?>"><span>Pay Later</span>
                                            </a></p>
                                    </div>
                                    <div class="row">
                                ...
                                <div class="mt-4 mx-4">
                                    ...
                                    <div class="form mt-3">
                                        ...
                                        <div class="inputbox"> <input type="text" name="name" min="1" max="999"
                                                class="form-control" required="required"> <span>Card Number</span> <i
                                                class="fa fa-eye"></i> </div>
                                        <div class="d-flex flex-row">
                                            <div class="inputbox"> <input type="text" name="name" min="1" max="999"
                                                    class="form-control" required="required"> <span>Expiration
                                                    Date</span> </div>
                                            <div class="inputbox ms-3"> <input type="text" name="name" min="1" max="999"
                                                    class="form-control" required="required"> <span>CVV</span> </div>
                                        </div>
                                        <div class="px-5 pay"> <button
                                                class="btn btn-primary btn-block proceed process_payment">Proceed
                                                To pay</button> </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane fade" id="paypal" role="tabpanel" aria-labelledby="paypal-tab">
                                <div class="px-5 mt-5">
                                    ...
                                    <div class="pay px-5"> <button
                                            class="btn btn-primary btn-block proceed process_payment">Proceed To
                                            pay</button> </div>
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    // Handle form submission when the "Proceed to Pay" button is clicked
    $(".process_payment").on("click", function() {
        // Submit the form with the ID "payment_form"
        $("#form_payment").submit();
    });
});
</script>
